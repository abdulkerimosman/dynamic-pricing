...
export default function Rakipler() {
  ...
  return (
    <div className="page-shell max-w-screen-xl">
      <div>
        <h1 className="page-title">Rakip Fiyat Takibi</h1>
        <p className="page-subtitle">Rakip sitelerden toplanan fiyat verileri</p>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              ...
              {!isLoading && fiyatlar.map((f) => (
                <tr key={f.rakip_fiyat_id} className="table-row">
                  ...
                  <td className="td text-gray-600">{f.Kanal?.kanal_adi ?? '—'}</td>
              ...
              {!isLoading && fiyatlar.length === 0 && (
                <tr><td colSpan={6} className="td text-center text-gray-500 py-10">Rakip fiyat verisi bulunamadı</td></tr>
              )}
            </tbody>
          </table>
        </div>
