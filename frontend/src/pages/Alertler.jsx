  ...
  return (
    <div className="space-y-5 max-w-screen-xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Alertler</h1>
          <p className="text-sm text-slate-500 mt-0.5">Sistem uyarıları ve aksiyon gerektiren durumlar</p>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-50">
              ...
              {!isLoading && alertler.map((a) => (
                <tr key={a.alert_id} className="tr-hover">
                  <td className="td font-medium">{a.KanalUrun?.Urun?.urun_adi ?? '—'}</td>
                  <td className="td text-slate-500">{a.KanalUrun?.Kanal?.kanal_adi ?? '—'}</td>
                  <td className="td">
                    <span className={a.durum === 'acik' ? 'badge-red' : 'badge-green'}>
                      {ALERT_TIPI_LABEL[a.alert_tipi] ?? a.alert_tipi}
                    </span>
                  </td>
                  <td className="td text-slate-500 max-w-xs">{a.mesaj}</td>
